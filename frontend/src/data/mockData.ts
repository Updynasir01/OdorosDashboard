// Demo/mock data when the API is unavailable or empty (e.g. Vercel without seeded DB)
...
export function getMockIndicators(regionId?: string): DroughtIndicator[] {
  ...
  const avgRainfall =
    SOMALIA_REGIONS_DATA.reduce((sum, r) => sum + r.rainfallDeficit, 0) / SOMALIA_REGIONS_DATA.length
  const avgNDVI =
    SOMALIA_REGIONS_DATA.reduce((sum, r) => sum + r.ndvi, 0) / SOMALIA_REGIONS_DATA.length
  const avgTemp =
    SOMALIA_REGIONS_DATA.reduce((sum, r) => sum + r.temperatureAnomaly, 0) / SOMALIA_REGIONS_DATA.length
  const avgWater =
    SOMALIA_REGIONS_DATA.reduce((sum, r) => sum + r.waterScarcity, 0) / SOMALIA_REGIONS_DATA.length
  const avgLivestock =
    SOMALIA_REGIONS_DATA.reduce((sum, r) => sum + r.livestockRisk, 0) / SOMALIA_REGIONS_DATA.length
...
/** Deterministic wobble so charts stay stable across renders */
function wobble(i: number, span: number): number {
  return (((i * 17 + 3) % span) / span) * 2 - 1
}
...
export function getMockTimeSeries(regionId: string, months: number): TimeSeriesData[] {
  ...
  for (let i = months; i >= 0; i--) {
    ...
    const monthFactor = (months - i) / Math.max(months, 1)
    const rainfall = Math.max(
      0,
      50 - region.rainfallDeficit * monthFactor + wobble(i, 20) * 10
    )
    const ndvi = Math.max(0.1, region.ndvi + 0.1 * monthFactor + wobble(i, 15) * 0.05)
    const temperature = 28 + region.temperatureAnomaly * monthFactor + wobble(i, 10) * 2
...
export function getMockImpactData(regionId?: string): ImpactData[] {
  const regions = regionId ? SOMALIA_REGIONS_DATA.filter((r) => r.id === regionId) : SOMALIA_REGIONS_DATA
  ...
  return regions.map((region) => {
    ...
    return {
      ...
      displacedHouseholds: Math.round((region.affectedPopulation * 0.15) / 6),
...
export function getMockPredictions(regionId?: string): Prediction[] {
  ...
  return regions.map((region) => {
    ...
    if (region.droughtLevel === 'emergency') {
      ...
      riskLevel = 'medium'
      factors = ['Uncertain rainfall patterns', 'Moderate temperature increase']
    }
