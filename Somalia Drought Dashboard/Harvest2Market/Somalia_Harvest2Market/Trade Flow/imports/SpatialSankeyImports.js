...
export const LegendImports = ({ smallThreshold, mediumThreshold, roundedMax }) => {
  return (
    <Card position="absolute" bottom={1} p={3} boxShadow="md">
      <VStack spacing={2} align="stretch">
        <HStack spacing={3}>
          <Box w="30px" h="5px" bg="teal.600" opacity={0.5} />
          <Text fontSize="xs"> ≤ {formatNumber(smallThreshold)} </Text>
        </HStack>
        <HStack spacing={3}>
          <Box w="30px" h="7px" bg="teal.600" opacity={0.5} />
          <Text fontSize="xs"> ≤ {formatNumber(mediumThreshold)} </Text>
        </HStack>
        <HStack spacing={3}>
          <Box w="30px" h="10px" bg="teal.600" opacity={0.5} />
...
export const SpatialSankeyImports = ({ data, smallThreshold, mediumThreshold, roundedMax, scaleFactor = 1 }) => {
    ...
    <>
      {data.map((trade, idx) => {
        ...
        return (
          <Line
            key={idx}
            to={trade.targetCoordinates}    // TO the importing country (selected)
            from={trade.sourceCoordinates}  // FROM the exporting countries
            stroke="#0d9488"
